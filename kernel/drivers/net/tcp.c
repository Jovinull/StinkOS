...
void tcp_handle(const void *payload, unsigned int len, ipv4_t src_ip)
{
	if (len < sizeof(struct tcp_hdr))
		return;
	/* RFC 1122 §4.2.3.10: TCP segments from broadcast / multicast /
	 * loopback / unspecified can never represent a legitimate peer.
	 * Silent drop is correct -- no state to update, no reply to send. */
	if (!ipv4_is_unicast(src_ip))
		return;
