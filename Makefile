...
$(BUILD)/anim.bin: apps/crt0.s apps/anim.c apps/libstink.h | $(BUILD)
	$(AS) -O0 apps/crt0.s -o $(BUILD)/crt0.o
	$(CC) $(CFLAGS) -c apps/anim.c -o $(BUILD)/anim_app.o
	$(LD) -Ttext 0x400000 --oformat binary -o $(BUILD)/anim.bin $(BUILD)/crt0.o $(BUILD)/anim_app.o

os: $(LINK_OBJS) linker.ld $(BUILD)/hello.bin $(BUILD)/box.bin $(BUILD)/fault.bin $(BUILD)/game.bin $(BUILD)/hi.bin $(BUILD)/anim.bin
