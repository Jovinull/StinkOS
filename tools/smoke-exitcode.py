#!/usr/bin/env python3
"""Validates that sys_exit_code(N) propagates through sys_wait() to
the parent. Drives shell to launch `exitcode` which forks, child
calls sys_exit_code(42), parent waits, asserts return value == 42.
Tier 2.3 milestone."""
import os, signal, socket, subprocess, sys, time

SER = "/tmp/stinkos_ec_ser.log"
MON = "/tmp/stinkos_ec_mon.sock"
SHELL_MENU_STEPS = 14
...
shellkeys("run exitcode"); sock.sendall(b"sendkey ret\n")
time.sleep(3.0)
...
checks = {
    "child forked":      "exitcode: child forked" in out,
    "wait returned 42":  "exitcode: wait returned 42" in out,
    "no FAIL path":      "exitcode: FAIL" not in out,
    "PASS marker":       "exitcode: PASS" in out,
...
print("PASS: sys_exit_code(N) propagates through sys_wait() to parent")
